...
void MultiWinScene::Initialize() {
    ...
    AddNewObject(new Engine::Label("player " + std::to_string(which_player_win) + " Win!", "pirulen.ttf", 48, halfW, halfH / 4 - 10, 255, 255, 255, 255, 0.5, 0.5));

    Engine::ImageButton *btn;
    btn = new Engine::ImageButton("win/dirt.png", "win/floor.png", halfW - 200, halfH * 7 / 4 - 50, 400, 100);
    btn->SetOnClickCallback(std::bind(&MultiWinScene::BackOnClick, this, 2));
    AddNewControlObject(btn);
    AddNewObject(new Engine::Label("Back", "pirulen.ttf", 48, halfW, halfH * 7 / 4, 0, 0, 0, 255, 0.5, 0.5));
    
    // TODO PROJECT-bonus (2): Add a text box in MultiWinScene to record the user’s name.
...
void MultiWinScene::Terminate() {
    ...
    if(tmp_file.is_open()) {
        ...
        std::ifstream scoreboard_file(scoreboard_name, std::ios::in);
        std::ofstream new_scoreboard_file(scoreboard_tmp);
        ...
        if(std::remove(scoreboard_name.c_str()) != 0){
...
void MultiWinScene::Update(float deltaTime) {
    ticks += deltaTime;
...
void MultiWinScene::BackOnClick(int stage) {
